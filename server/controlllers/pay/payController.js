...
exports.payFunction = async (req, res) => {
  try {
    ...
    const data = {
      ...
      amount: amount * 100,
      redirectUrl: `https://backend.twicks.in/api/pay/checkStatus?transactionId=${merchantTransactionId}&cartId=${cartId}&street=${street}&city=${city}&country=${country}&state=${state}&zipCode=${zipCode}&landmark=${landmark}`, //url to be redirected post complete transaction
