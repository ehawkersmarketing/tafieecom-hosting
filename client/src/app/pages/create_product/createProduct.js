...
const CreateProduct = () =>{
  ...
  const[image , setImage] = useState();
    ...
    const [inputHandler, setInputHandler] = useState({title:'',
        description:'' , price:'', quantity:'', productType:'', maxQuantity:maxValue , minQuantity:value
      });
      ...
      const onSubmitHandler = async (e) => {
        ...
        const { title , description, maxQuantity , minQuantity, price ,quantity, productType } = inputHandler;
        console.log(inputHandler)
        //  console.log(image)
        if (title === "") {
          alert("Add Title"); 
      ...
      const { data } = await axios.post('http://localhost:8080/api/createProduct', formData);
      if (data.success) {
